const ytBlockSettings = (function () {
  // See README.md for details on these
  const mainFrontPageVideoTag = 'ytd-rich-item-renderer'
  const mainFrontPageElToObserve = 'primary'
  const mainVideoSearchTag = 'ytd-video-renderer'
  const mainVideoSearchElToObserve = 'contents'
  const mainSideBarVideoTag = 'ytd-compact-video-renderer'
  const mainSideBarVideoElToObserve = 'related'
  const urlYtSearch = 'results?search_query='
  const urlYtWatch = 'watch?v='
...
const ytBlockUser = (function () {
  ...
  const setBlock = function (type, val) {
    ...
    function pushChromeVal (_chromeData) {
      if (!Array.isArray(_chromeData)) { return null }

      const chromeData = [..._chromeData]

      if (val !== '') { chromeData.push(val) }
  ...
  const delBlock = function (type, val) {
    // TODO
  }

  // Gets blocks in memory, not Chrome storage
  const getMemBlocks = function (type) {
    if (type === 'url') {
      return urlBlocks
...
const ytBlock = (function (_user, _ytBlockSettings) {
  const user = _user
  ...
  const init = function () {
    if (!initialized) {
      user.init()
      const webResources = document.createElement('web.js')
